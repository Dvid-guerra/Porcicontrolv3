...
function Porcicontrol({ user, db, appId }) {
  ...
  const [cronogramaSanidadBase, , l6] = useCloudStorage('agro_cronogramabase', INITIAL_CRONOGRAMA_SANIDAD, user, db, appId);
  ...
  const [, setTareasSanidad, l15] = useCloudStorage('agro_tareassanidad', [], user, db, appId);
...
${JSON.stringify(contextoGranja)}`;
  ...
  const handleIngresoLote = async (e) => {
    ...
    const tareasHeredadas = listCronoBase.map((t) => ({ id: crypto.randomUUID(), loteId: nl.id, dia: t.dia, tarea: t.tarea }));
    try {
  ...
  const handleGuardarTolva = (e) => {
    ...
    setAlimentos(prev => [...safeArr(prev), {
      id: crypto.randomUUID(), loteId: loteActivo.id, fecha: fd.get('fecha'),
                ...
                <div key={lote.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden group flex flex-col">
                  ...
                  <div className="p-5 space-y-4 flex-1">
                    <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                      ...
                      <span className="font-bold text-slate-800">{st.ventasLote.reduce((total, sale) => total + (sale.cantidadCerdos || 0), 0)} <span className="text-[10px] font-normal text-slate-400 uppercase">de {lote.cantidad}</span></span>
                    </div>
                    <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
    ...
    if (!lote) {
      return (
        <div className="p-8 text-center">
          <p className="text-slate-600 mb-4">No hay un lote histórico seleccionado.</p>
          <button onClick={() => setVista('historial')} className="bg-slate-800 text-white px-5 py-3 rounded-xl">Volver al historial</button>
        </div>
      );
    }
    ...
    return (
      <div className="p-4 md:p-8 max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6 print:hidden">
          <button onClick={() => setVista('historial')} className="text-indigo-700 font-bold flex items-center">
            <ChevronRight size={20} className="rotate-180 mr-1" /> Volver
          </button>
          <button onClick={() => descargarPDF('historical-report', `lote-${lote.id}.pdf`)} className="bg-indigo-600 text-white px-5 py-3 rounded-xl font-bold flex items-center">
            <Download size={18} className="mr-2" /> Descargar PDF
          </button>
        </div>

        <section id="historical-report" className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-slate-200">
          <header className="border-b-4 border-slate-900 pb-6 mb-8">
            <p className="text-xs uppercase tracking-widest font-black text-indigo-600">Reporte final de trazabilidad</p>
            <h2 className="text-3xl font-black text-slate-900 mt-2">{corral?.nombre || 'Corral eliminado'}</h2>
            <p className="text-slate-500 mt-1">{lote.fechaIngreso} — {lote.fechaSalida || 'Sin fecha de cierre'}</p>
          </header>

          {stats.fechaInvalida && <div className="mb-6 p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl font-bold">Las fechas del lote son inconsistentes y deben corregirse.</div>}

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ['Animales iniciales', lote.cantidad || 0],
              ['Animales vendidos', soldCount],
              ['Mortalidad', `${stats.mortalidadPorcentaje.toFixed(1)}%`],
              ['FCA estimado', stats.fca.toFixed(2)]
            ].map(([label, value]) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="border border-slate-200 rounded-xl p-5">
              <h3 className="font-black text-slate-800 mb-4">Resultados productivos</h3>
              <p>Duración: <strong>{stats.diasLote} días</strong></p>
              <p>Peso inicial: <strong>{stats.pesoInicial.toFixed(1)} lb</strong></p>
              <p>Peso final: <strong>{stats.pesoActual.toFixed(1)} lb</strong></p>
              <p>Libras vendidas: <strong>{stats.librasVendidas.toFixed(1)} lb</strong></p>
              <p>Peso producido para FCA/GMD: <strong>{stats.pesoProducidoLb.toFixed(1)} lb</strong></p>
              <p>Alimento registrado: <strong>{stats.consumoTotalLb.toFixed(1)} lb</strong></p>
            </div>
            <div className="border border-slate-200 rounded-xl p-5">
              <h3 className="font-black text-slate-800 mb-4">Resultados financieros</h3>
              <p>Costo acumulado: <strong>{formatearMoneda(stats.costoInvertidoTotal + (lote.manoObra || 0))}</strong></p>
              <p>Ventas reales: <strong>{formatearMoneda(stats.ingresoRealVentas)}</strong></p>
              <p>Valor estimado vivo: <strong>{formatearMoneda(stats.ingresoEstimadoVivos)}</strong></p>
              <p>Ingresos totales: <strong>{formatearMoneda(stats.ingresoEstimado)}</strong></p>
              <p className={`text-xl mt-3 ${stats.utilidadNeta >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>Utilidad neta: <strong>{formatearMoneda(stats.utilidadNeta)}</strong></p>
            </div>
          </div>
        </section>
      </div>
            ...
            {listCatAlimentos.map(item => (
              <div key={item.id} className="flex flex-col p-3 border border-slate-100 rounded-lg bg-slate-50 group">
                ...
                <div className="flex items-center gap-2 w-full justify-between">
                  <div className="flex items-center">
                    <input type="number" step="1" value={item.librasPorSaco} onChange={(e) => setCatalogoAlimento(prev => safeArr(prev).map(i => i.id === item.id ? { ...i, librasPorSaco: parseFloat(e.target.value) || 0 } : i))} className="w-14 p-1.5 text-center text-sm border border-slate-300 rounded outline-none focus:ring-2 focus:ring-emerald-500 bg-white" />
                    <span className="text-slate-400 text-xs ml-1">lbs</span>
                  </div>
                  <div className="flex items-center">
                    <span className="text-slate-400 text-sm mr-1">Q</span>
                    <input type="number" value={item.precioSaco} onChange={(e) => setCatalogoAlimento(prev => safeArr(prev).map(i => i.id === item.id ? { ...i, precioSaco: parseFloat(e.target.value) || 0 } : i))} className="w-20 p-1.5 text-right font-bold text-blue-700 border border-slate-300 rounded outline-none focus:ring-2 focus:ring-blue-500 bg-white" />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-2 flex-1">
            {listCatIngredientes.map(item => (
              <div key={item.id} className="flex flex-col p-3 border border-slate-100 rounded-lg bg-slate-50 group">
                ...
                <div className="flex items-center gap-2 w-full justify-between">
                  <div className="flex items-center">
                    <input type="number" step="1" value={item.librasPorSaco} onChange={(e) => setCatalogoIngredientes(prev => safeArr(prev).map(i => i.id === item.id ? { ...i, librasPorSaco: parseFloat(e.target.value) || 0 } : i))} className="w-14 p-1.5 text-center text-sm border border-slate-300 rounded outline-none focus:ring-2 focus:ring-emerald-500 bg-white" />
                    <span className="text-slate-400 text-xs ml-1">lbs</span>
                  </div>
                  <div className="flex items-center">
                    <span className="text-slate-400 text-sm mr-1">Q</span>
                    <input type="number" value={item.precioSaco} onChange={(e) => setCatalogoIngredientes(prev => safeArr(prev).map(i => i.id === item.id ? { ...i, precioSaco: parseFloat(e.target.value) || 0 } : i))} className="w-20 p-1.5 text-right font-bold text-amber-700 border border-slate-300 rounded outline-none focus:ring-2 focus:ring-amber-500 bg-white" />
                  </div>
                </div>
              </div>
            ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-300">
                  <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center"><span className="text-slate-600 font-medium">Animales Vivos:</span><span className={`font-black text-xl ${stats.mortalidadPorcentaje > 5 ? 'text-rose-500' : 'text-slate-800'}`}>{stats.cantidadActual} <span className="text-sm font-normal text-slate-500">({stats.cantidadVendida} vendidos · {stats.mortalidadPorcentaje.toFixed(1)}% mortalidad)</span></span></div>
                      ...
                      <div className="flex justify-between items-center"><span className="text-slate-600 font-medium">Peso producido:</span><span className="font-bold text-slate-800">{stats.pesoProducidoLb.toFixed(1)} lbs <span className="text-xs text-slate-400">({stats.librasVendidas.toFixed(1)} lbs vendidas)</span></span></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-300">
                  <div className="space-y-8">
                    <div>
                      ...
                      <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.target); setVacunas(prev => [...safeArr(prev), { id: crypto.randomUUID(), loteId: loteActivo.id, fecha: fd.get('fecha'), nombre: fd.get('nombre').trim(), costo: parseFloat(fd.get('costo')) || 0 }]); e.target.reset(); }} className="space-y-4 bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.target); setGastosExtra(prev => [...safeArr(prev), { id: crypto.randomUUID(), loteId: loteActivo.id, fecha: fd.get('fecha'), descripcion: fd.get('descripcion').trim(), monto: parseFloat(fd.get('monto')) }]); e.target.reset(); }} className="space-y-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.target); const cant = parseInt(fd.get('cantidad'), 10); if (cant <= 0 || cant > stats.cantidadActual) return mostrarAlerta("Error", "La baja debe ser mayor a cero y no superar los cerdos vivos."); setBajas(prev => [...safeArr(prev), { id: crypto.randomUUID(), loteId: loteActivo.id, fecha: fd.get('fecha'), causa: fd.get('causa'), cantidad: cant }]); e.target.reset(); }} className="space-y-4 bg-rose-50/50 p-6 rounded-2xl border border-rose-100 mb-8">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-12 w-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
                  <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl text-center">
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-2">Cerdos Vendidos / Vivos</p>
                    <p className="text-3xl font-black text-slate-800">{st.cantidadVendida} <span className="text-base font-normal text-slate-500">vendidos · {st.cantidadActual} vivos</span></p>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl text-center">
                ...
                <div className="mb-10 page-break-inside-avoid">
                  ...
                  <table className="w-full text-left text-base">
                    <tbody>
                      ...
                      <tr className="border-b border-slate-100"><td className="py-3 text-slate-600">Libras Vendidas por Báscula</td><td className="py-3 font-semibold">{st.librasVendidas.toFixed(1)} lbs</td></tr>
                      <tr className="border-b border-slate-100"><td className="py-3 text-slate-600">Peso Producido para FCA/GMD</td><td className="py-3 font-semibold">{st.pesoProducidoLb.toFixed(1)} lbs</td></tr>
                ...
                <div className="mb-10 page-break-inside-avoid">
                  ...
                  <table className="w-full text-left text-base">
                    <tbody>
                      ...
                      <tr className="border-b border-slate-100"><td className="py-3 text-slate-800 font-bold">Ventas reales por báscula</td><td className="py-3 font-black text-right text-emerald-600 text-xl">+{formatearMoneda(st.ingresoRealVentas)}</td></tr>
                      <tr className="border-b border-slate-200"><td className="py-3 text-slate-800 font-bold">Valor estimado de animales vivos</td><td className="py-3 font-black text-right text-emerald-600 text-xl">+{formatearMoneda(st.ingresoEstimadoVivos)}</td></tr>
    ...
    return (
      <div className="p-4 md:p-6 space-y-6 max-w-screen-xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className={`border rounded-xl p-4 shadow-sm ${totGlobal.utilidadRealVentas >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'
            }`}>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Utilidad Realizada</p>
            <p className={`text-2xl font-black ${totGlobal.utilidadRealVentas >= 0 ? 'text-emerald-700' : 'text-red-600'
              }`}>{formatearMoneda(totGlobal.utilidadRealVentas)}</p>
            <p className={`text-xs font-bold mt-2 ${totGlobal.utilidadRealVentas >= 0 ? 'text-emerald-600' : 'text-red-500'
              }`}>Margen: {margenGlobal.toFixed(1)}%</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Utilidad Estimada</p>
            <p className={`text-2xl font-black ${totGlobal.utilidadNeta >= 0 ? 'text-indigo-700' : 'text-red-600'}`}>
              {formatearMoneda(totGlobal.utilidadNeta)}
            </p>
            <p className="text-xs text-slate-500 mt-2">Incluye {formatearMoneda(totGlobal.valorEstimadoVivos)} en animales vivos</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-500">Estructura de Costos Acumulada</h3>
            <span className="text-xs font-bold text-slate-500 bg-slate-100 border border-slate-200 px-3 py-1 rounded-lg">
              Costo/lb vendida: {costoPorLibraGlobal > 0 ? formatearMoneda(costoPorLibraGlobal) : '—'}
            </span>
          </div>
          <div className="space-y-3">
            {costItems.map(item => {
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="py-3 px-4 font-bold text-right text-slate-600">Animales</th>
                    ...
                    <th className="py-3 px-4 font-bold text-right text-slate-600">Utilidad Real</th>
                    <th className="py-3 px-4 font-bold text-right text-slate-600">Estimada</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                    ...
                    return (
                      <React.Fragment key={r.id}>
                        <tr
                          ...
                          className={`cursor-pointer transition-colors ${estaAbierto ? 'bg-emerald-50/60' : 'hover:bg-slate-50'}`}
                        >
                          <td className="py-3 px-4">
                            <p className="font-bold text-slate-800 flex items-center gap-2">
                              <ChevronRight size={16} className={`text-emerald-600 transition-transform ${estaAbierto ? 'rotate-90' : ''}`} />
                              {r.nombre}
                            </p>
                            <p className="text-xs text-slate-400 ml-6">{r.cantidadInicial} lechones · {r.fechaIngreso} · {r.diasLote} dias</p>
                          </td>
                          <td className="py-3 px-4">
                          ...
                          <td className="py-3 px-4 text-right text-xs font-bold text-slate-600">
                            ...
                            <div className="text-slate-400">{r.cerdosVendidos} vendidos · {r.bajasTotales} bajas</div>
                          </td>
                          ...
                          <td className={`py-3 px-4 text-right font-bold ${r.utilidadRealVentas >= 0 ? 'text-emerald-700' : 'text-red-600'
                            }`}>{formatearMoneda(r.utilidadRealVentas)}</td>
                          <td className={`py-3 px-4 text-right font-bold ${r.utilidadNeta >= 0 ? 'text-indigo-700' : 'text-red-600'
                            }`}>{formatearMoneda(r.utilidadNeta)}</td>
                        </tr>
                        {estaAbierto && (
                          <tr className="bg-white">
                            <td colSpan={7} className="px-4 pb-6">
                              <div className="border border-emerald-100 bg-emerald-50/30 rounded-xl p-4 md:p-5 space-y-5">
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                  <div className="bg-white border border-slate-200 rounded-lg p-3">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Costo por cerdo</p>
                                    <p className="text-xl font-black text-slate-800">{formatearMoneda(r.costoPorCerdoOperado)}</p>
                                    <p className="text-xs text-slate-500">Promedio historico operado</p>
                                  </div>
                                  <div className="bg-white border border-slate-200 rounded-lg p-3">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Costo de restantes</p>
                                    <p className="text-xl font-black text-amber-600">{formatearMoneda(r.costoAsignadoRestantes)}</p>
                                    <p className="text-xs text-slate-500">{r.cerdosRestantes} cerdos vivos</p>
                                  </div>
                                  <div className="bg-white border border-slate-200 rounded-lg p-3">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Costo futuro restante</p>
                                    <p className="text-xl font-black text-rose-600">{formatearMoneda(r.costoFuturoRestantes)}</p>
                                    <p className="text-xs text-slate-500">Alimento estimado hasta meta</p>
                                  </div>
                                  <div className="bg-white border border-slate-200 rounded-lg p-3">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Equilibrio restantes</p>
                                    <p className="text-xl font-black text-indigo-700">{r.puntoEquilibrioRestantes > 0 ? formatearMoneda(r.puntoEquilibrioRestantes) : '—'}</p>
                                    <p className="text-xs text-slate-500">Precio minimo por libra</p>
                                  </div>
                                </div>

                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                  <div className="bg-white border border-slate-200 rounded-lg p-4">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Costos acumulados</h4>
                                    <div className="space-y-2 text-sm">
                                      ...
                                      <div className="flex justify-between"><span className="text-slate-500">Alimento</span><strong>{formatearMoneda(r.costoAlimento)}</strong></div>
                                      <div className="flex justify-between"><span className="text-slate-500">Medicinas</span><strong>{formatearMoneda(r.costoMedicinas)}</strong></div>
                                      <div className="flex justify-between"><span className="text-slate-500">Gastos extra</span><strong>{formatearMoneda(r.costoGastos)}</strong></div>
                                      ...
                                      <div className="flex justify-between border-t border-slate-200 pt-2 font-black"><span>Total</span><span>{formatearMoneda(r.inversionTotal)}</span></div>
                                    </div>
                                  </div>

                                  <div className="bg-white border border-slate-200 rounded-lg p-4">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Lectura del lote</h4>
                                    <div className="space-y-2 text-sm">
                                      <div className="flex justify-between"><span className="text-slate-500">Peso actual</span><strong>{r.pesoActual > 0 ? `${r.pesoActual.toFixed(1)} lb` : 'Sin peso'}</strong></div>
                                      <div className="flex justify-between"><span className="text-slate-500">Meta de peso</span><strong>{r.pesoObjetivo.toFixed(0)} lb</strong></div>
                                      <div className="flex justify-between"><span className="text-slate-500">FCA</span><strong>{r.fca > 0 ? r.fca.toFixed(2) : 'Sin dato'}</strong></div>
                                      <div className="flex justify-between"><span className="text-slate-500">Libras vendidas</span><strong>{r.lbsCarneVendida.toFixed(1)} lb</strong></div>
                                      ...
                                      <div className="flex justify-between border-t border-slate-200 pt-2"><span className="text-slate-500">Utilidad proyectada restantes</span><strong className={r.utilidadProyectadaRestantes >= 0 ? 'text-emerald-700' : 'text-rose-600'}>{formatearMoneda(r.utilidadProyectadaRestantes)}</strong></div>
                                    </div>
                                  </div>

                                  <div className="bg-slate-900 text-white rounded-lg p-4">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-emerald-300 mb-3">Decision financiera</h4>
                                    <p className="text-sm leading-relaxed text-slate-300">
                                      Este lote ya genero <strong className="text-emerald-300">{formatearMoneda(r.ingresosBrutos)}</strong> en ventas reales.
                                      Los <strong className="text-white">{r.cerdosRestantes}</strong> cerdos restantes cargan un costo historico de <strong className="text-amber-300">{formatearMoneda(r.costoAsignadoRestantes)}</strong>
                                      {r.costoFuturoRestantes > 0 ? <> y requieren aprox. <strong className="text-rose-300">{formatearMoneda(r.costoFuturoRestantes)}</strong> mas para llegar a meta.</> : <> y no tienen costo futuro proyectado por peso.</>}
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
                                  <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                                    <h4 className="text-xs font-black uppercase tracking-widest text-slate-500">Ventas parciales por bascula</h4>
                                    <span className="text-xs font-bold text-slate-400">{r.ventasDetalle.length} registros</span>
                                  </div>
                                  {r.ventasDetalle.length === 0 ? (
                                    <div className="p-5 text-sm text-slate-400 italic text-center">Este lote aun no tiene ventas parciales registradas.</div>
                                  ) : (
                                    <div className="overflow-x-auto">
                                      <table className="w-full text-xs text-left">
                                        <thead className="bg-slate-50 text-slate-500">
                                          <tr>
                                            ...
                                            <th className="py-2 px-3 text-right">Cerdos</th>
                                            <th className="py-2 px-3 text-right">Libras</th>
                                            <th className="py-2 px-3 text-right">Promedio</th>
                                            <th className="py-2 px-3 text-right">Precio/lb</th>
                                            <th className="py-2 px-3 text-right">Venta</th>
                                            <th className="py-2 px-3 text-right">Costo asignado</th>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                          {r.ventasDetalle.map(venta => (
                                            <tr key={venta.id}>
                                              <td className="py-2 px-3 font-bold text-slate-700">{venta.fecha || '—'}</td>
                                              <td className="py-2 px-3 text-right">{venta.cantidadCerdos || 0}</td>
                                              <td className="py-2 px-3 text-right">{venta.libras.toFixed(1)}</td>
                                              <td className="py-2 px-3 text-right">{venta.pesoPromedio.toFixed(1)}</td>
                                              <td className="py-2 px-3 text-right">{formatearMoneda(venta.precioLibra || 0)}</td>
                                              <td className="py-2 px-3 text-right font-bold text-emerald-700">{formatearMoneda(venta.totalVenta || 0)}</td>
                                              <td className="py-2 px-3 text-right font-bold text-amber-700">{formatearMoneda(venta.costoAsignado)}</td>
                                              <td className={`py-2 px-3 text-right font-black ${venta.utilidadEstimada >= 0 ? 'text-emerald-700' : 'text-rose-600'}`}>{formatearMoneda(venta.utilidadEstimada)}</td>
                                            </tr>
                                          ))}
                                        </tbody>
                                      </table>
                ...
                <tfoot className="bg-slate-100 border-t-2 border-slate-300">
                  <tr>
                    <td colSpan={3} className="py-3 px-4 font-black text-slate-700 uppercase text-xs tracking-widest">TOTALES GLOBALES</td>
                    ...
                    <td className={`py-3 px-4 text-right font-black ${totGlobal.utilidadRealVentas >= 0 ? 'text-emerald-700' : 'text-red-600'
                      }`}>{formatearMoneda(totGlobal.utilidadRealVentas)}</td>
                    <td className={`py-3 px-4 text-right font-black ${totGlobal.utilidadNeta >= 0 ? 'text-indigo-700' : 'text-red-600'
                  ...
                  chatMessages.map((msg, idx) => (
                    <div key={idx} className={`p-3 rounded-xl max-w-[85%] shrink-0 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-emerald-600 text-white self-end rounded-tr-none shadow-md' : 'bg-white border border-slate-200 text-slate-700 self-start rounded-tl-none shadow-sm'}`}>
                      {msg.role === 'user' ? (
                        ...
                        <ReactMarkdown 
                          components={{
                            strong: ({node: _node, ...props}) => <strong className="font-bold text-emerald-800" {...props} />,
                            ul: ({node: _node, ...props}) => <ul className="list-disc pl-4 my-2" {...props} />,
                            ol: ({node: _node, ...props}) => <ol className="list-decimal pl-4 my-2" {...props} />,
                            ...
                            h3: ({node: _node, ...props}) => <h3 className="text-md font-bold my-1" {...props} />,
...
export default function App() {
  ...
  return <Porcicontrol user={user} db={db} appId={globalAppId} role={membership.role} />;
}
